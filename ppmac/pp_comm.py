...
try:
    import fast_gather as fast_gather_mod
...
class ShellChannel(object):
    ...
    def send_line(self, line, delim='\n', sync=False):
        ...
        with self.lock:
            ...
            channel.send('%s%s' % (line, delim)) #the output here is an int
...
class GpasciiChannel(ShellChannel):
    ...
    def readVal(self, command, timeout=2.0):
    ...
    def send_program(self, coord, prog_num, motors={},
        ...
        if run:
            try:
                self.run_and_wait(coord, prog_num)
            except GPError as ex:
...
class PPComm(object):
    ...
    def gpascii_file(self, filename, check_errors=True):
        """
        Execute a gpascii script by remote filename
        """
        ret = self.shell_command('gpascii -i"%s" 2>&1' % filename)
        if not check_errors:
            return ret
    ...
    def shell_command(self, command, verbose=False):
        """
    ...
    def shell_output(self, command, wait_match=None, timeout=None):
        """
        ...
        if wait_match is not None:
            for line, m in _wait_for(stdout.readlines(), wait_match):
                yield line, m
        else:
            for line in stdout.readlines():
                yield line.rstrip('\n')
...
def main():
    comm = PPComm(host='192.168.0.200', port=22, user='root', password='deltatau')

    #channel = comm.shell_output('ls',None,None)
